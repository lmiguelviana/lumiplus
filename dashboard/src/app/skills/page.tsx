...
import {
  ...
  LayoutGrid, Mail, CreditCard, FileText, Loader2, Check, X, Zap, Trash2, Bot, Instagram, ImageUp, Settings2, Key,
} from 'lucide-react';
import api from '@/lib/api';

const ICON_MAP: Record<string, any> = {
  Search, BookOpen, Globe, Brain, UserPlus, Sparkles, Calendar, Table,
  LayoutGrid, Mail, CreditCard, FileText, Zap, Instagram, ImageUp,
};
...
export default function SkillsPage() {
  ...
  const [configModal, setConfigModal] = useState<CustomSkill | null>(null);
  ...
  const openConfigModal = (cs: CustomSkill) => {
    setConfigModal(cs);
    setCredValue('');
    setCredSaved(false);
  };
  ...
  const handleSaveCred = async () => {
    if (!configModal || !credValue.trim()) return;
    setSavingCred(true);
    try {
      await api.put(`/settings/${configModal.credentialKey}`, { value: credValue.trim() });
      setCredSaved(true);
      setTimeout(() => { setCredSaved(false); setConfigModal(null); }, 1500);
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 pt-10 pb-20 space-y-6">
            ...
            {filtered.map(skill => {
              ...
              return (
                ...
                >
                  ...
                  {skill.isDefault && (
                    <div className="mt-2 px-2 py-0.5 bg-primary/10 border border-primary/20 text-primary text-[9px] font-bold uppercase tracking-widest inline-block">
                      Padrao
                    </div>
                  )}
                  {skill.comingSoon && (
                ...
                <div key={cs.skillId} className="industrial-card p-5 border-purple-500/20">
                  ...
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <button
                        onClick={() => openConfigModal(cs)}
                        className="p-1.5 rounded border border-purple-500/30 bg-purple-500/10 text-purple-400 hover:bg-purple-500/20 transition-all"
                        title="Configurar credencial"
                      >
      ...
      {/* Modal de configuração de credencial (Personalizadas) */}
      {configModal && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={() => setConfigModal(null)}>
          <div className="bg-background border border-border-strong max-w-md w-full p-6 space-y-4" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <Key className="w-5 h-5 text-purple-400" />
                <div>
                  <h2 className="font-black text-sm uppercase tracking-widest">{configModal.apiName}</h2>
                  <p className="text-[9px] text-muted font-mono">{configModal.credentialKey}</p>
                </div>
              </div>
              <button onClick={() => setConfigModal(null)} className="text-muted hover:text-foreground">
                <X className="w-4 h-4" />
              </button>
            </div>

            <p className="text-xs text-foreground/50">
              Insira o valor da credencial <strong>{configModal.credentialKey}</strong>.
              Ela será salva de forma segura e criptografada no vault do workspace.
            </p>
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                onKeyDown={e => e.key === 'Enter' && handleSaveCred()}
                placeholder={`Cole aqui o valor de ${configModal.credentialKey}...`}
                className="w-full h-10 px-3 text-xs font-mono bg-surface border border-border-strong focus:border-purple-500 focus:outline-none"
                style={{ borderRadius: 0 }}
                autoFocus
              />
            </div>
            ...
            <div className="flex gap-2 pt-1">
              ...
              <button
                ...
                className="flex-1 py-2.5 text-[10px] font-black uppercase tracking-widest bg-purple-600 text-white hover:bg-purple-500 transition-colors flex items-center justify-center gap-2 disabled:opacity-50"
              >
                {savingCred ? <Loader2 className="w-3 h-3 animate-spin" /> : credSaved ? <Check className="w-3 h-3" /> : <Key className="w-3 h-3" />}
                {credSaved ? 'Salvo!' : 'Salvar Credencial'}
              </button>
            </div>
          </div>
        </div>
